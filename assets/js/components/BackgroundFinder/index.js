...
export default function BackgroundFinder({selectedBackground, setSelectedBackground}) {
    ...
    return (
        <>
            <Combobox as="div" value={selectedBackground} onChange={setSelectedBackground}>
                ...
                <div className="relative mt-1">
                    <Combobox.Input
                        ...
                        displayValue={(background) => background.title}
                    />
                                    ...
                                    {({ active, selected }) => (
                                        <>
                                            <div className="flex items-center">
                                                <img src={background.contentUrl} alt="" className="h-6 w-6 flex-shrink-0 rounded-full" />
                                                <span className={classNames('ml-3 truncate', selected && 'font-semibold')}>{background.title}</span>
                                            </div>
            ...
            <div>
                {selectedBackground && <img src={selectedBackground.contentUrl} alt={selectedBackground.title} className="mt-2" />}
            </div>
        </>
    )
}
